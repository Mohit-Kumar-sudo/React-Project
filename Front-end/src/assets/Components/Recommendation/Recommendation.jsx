...
const Recommendation = (props) => {
    ...
    React.useEffect(() => {
        if(rankingList.length > 0){
            Axios.get(`http://www.omdbapi.com/?s=${rankingList[rankingList.length-1].Title}&apikey=1bacbed6&`)
            .then((res)=>{
                if(res.data.Search)
                ...
                flexWrap:"wrap"}}>
                    {
                        result.map((item,i)=>{
                            return(
                                <div className="col-md-12 col-lg-6" id={i} onDragStart={(e)=>{
                                        var data = JSON.stringify(item);
                                        e.dataTransfer.setData('Text/html', data);
                                      }} draggable="true">
                                    <div className="card" onClick={()=>{handleAddition(item)}}>
